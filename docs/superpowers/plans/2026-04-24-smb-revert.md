# Revert to Share Name Requirement

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Revert the complex multi-share and root-discovery logic, returning to a simpler model where the user must provide a Share name in the URL for SMB connections (required on macOS). 

**Architecture:** Remove `smb-rs` crate usage and dependencies. Simplify `SmbStorage` back to a single `mount_point` per connection. Keep the `base_path` feature so users can still specify deep subfolders like `192.168.2.200/OST_NAS/AI绘画`.

**Tech Stack:** Rust, Tauri

---

### Task 1: Clean up Cargo.toml

**Files:**
- Modify: `src-tauri/Cargo.toml`

- [ ] **Step 1: Remove smb crate**
Remove `smb` and `smb-rpc` dependencies.

- [ ] **Step 2: Commit**

```bash
git add src-tauri/Cargo.toml
git commit -m "chore: remove smb crate dependencies"
```

### Task 2: Revert SmbStorage to Single Mount Point

**Files:**
- Modify: `src-tauri/src/vfs/smb.rs`

- [ ] **Step 1: Revert imports and struct**
...
- [ ] **Step 3: Restore `ensure_connected`**
...
- [ ] **Step 4: Simplify local path resolution**
...
git commit -m "refactor(smb): revert to single mount point and remove smb-rs dependency"
```

### Task 3: Restore VFS Trait Implementations

**Files:**
- Modify: `src-tauri/src/vfs/smb.rs`

- [ ] **Step 1: Restore trait methods**
...
```bash
git add src-tauri/src/vfs/smb.rs
git commit -m "fix(smb): restore VFS operations using native mount point"
```